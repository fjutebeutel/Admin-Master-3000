...
function Invoke-Updates {
    ...
    try {
        ...
        if ($updates) {
            Write-Host "$($updates.Count) Updates verfügbar." -ForegroundColor Yellow
            foreach ($u in $updates) {
                Write-Host "`nTitel: $($u.Title)"
                if ($u.KBArticleIDs) { Write-Host "KB:    $($u.KBArticleIDs -join ', ')" }
                if ($u.Size) { Write-Host "Größe: $($u.Size) MB" }
                ...
                if ($answer -match '^[JjYy]$') {
                    ...
                    Write-Host "Übersprungen: $($u.Title)" -ForegroundColor Yellow
                }
            }
        } else {
